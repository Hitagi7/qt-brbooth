...
void Camera::startCamera()
{
    ...
    // Try opening with index 0 first, then 1 if 0 fails.
    // This makes it more robust for different systems.
    bool opened = false;
    for (int i = 0; i < 2; ++i) { // Try index 0 and 1
        if (cap.open(i, cv::CAP_ANY)) {
            opened = true;
            qDebug() << "Camera: Successfully opened camera with index:" << i;
            break;
        }
        qWarning() << "Camera: Could not open camera with index:" << i;
    }

    if (!opened) {
        qWarning() << "Camera: Error: Could not open any camera.";
    ...
    if (cap.read(firstFrame) && !firstFrame.empty()) {
        ...
        if (!firstImage.isNull()) {
            ...
            qDebug() << "Camera: First frame emitted immediately. Size:" << firstImage.size();
        } else {
            qWarning() << "Camera: First frame QImage conversion failed.";
        }
    } else {
        qWarning() << "Camera: Failed to read first frame immediately or frame is empty.";
    }
    ...
    if (actual_fps > 0) {
        ...
        timerInterval = 33; // Default to ~30 FPS if actual_fps is 0
...
void Camera::processFrame()
{
    ...
    if (!cap.isOpened() || !cap.read(frame) || frame.empty()) {
        qWarning() << "Camera: Failed to read frame from camera or frame is empty in processFrame.";
        // loopTimer.restart();
        return;
    }
    ...
    if (image.isNull()) {
        qWarning() << "Camera: Failed to convert cv::Mat to QImage in processFrame.";
        // loopTimer.restart();
        return;
    }
