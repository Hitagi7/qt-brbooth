...
public:
    ...
    // Watershed segmentation is always enabled when segmentation is active
...
private:
    ...
    // Watershed segmentation helpers
    cv::Mat createWatershedPersonMask(const cv::Mat &frame) const;
    cv::UMat createWatershedPersonMaskGPU(const cv::UMat &gpuFrame) const;
    ...
    // Watershed segmentation configuration (no additional configuration needed)
