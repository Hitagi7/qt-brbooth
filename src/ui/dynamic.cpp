...
#include "ui_dynamic.h"  // Full definition of Ui::Dynamic class (generated by Qt)
    ...
    , fullscreenPlayer(nullptr)
{
    ...
    fullscreenStackWidget->setStyleSheet("background-color: black;"); // Black background for fullscreen
    ...
    // Install event filters for click detection on fullscreen video, stack widget, and the main widget
    fullscreenVideoWidget->installEventFilter(this);
    fullscreenStackWidget->installEventFilter(this);
    this->installEventFilter(this);
    qDebug() << "Dynamic::Dynamic - Fullscreen video widget, stack widget, and main widget event filters installed.";
...
{
    // Find project root by looking for qt-brbooth.pro file
    QString projectRoot;
    ...
    // If project root not found, try to use current directory or application directory
    if (projectRoot.isEmpty()) {
        projectRoot = QDir::currentPath();
        qWarning() << "Project root not found, using current directory:" << projectRoot;
    }
    
    // Template videos from project root templates/dynamic/ directory
    QString templatesDir = projectRoot + "/templates/dynamic";
    QStringList actualVideoPaths;
    actualVideoPaths << templatesDir + "/vidtemplate1.mp4"
                     << templatesDir + "/vidtemplate2.mp4"
                     << templatesDir + "/vidtemplate3.mp4"
                     << templatesDir + "/vidtemplate4.mp4"
                     << templatesDir + "/vidtemplate5.mp4";
    
    qDebug() << "Dynamic::setupVideoPlayers - Using templates directory:" << templatesDir;
    ...
    for (int i = 0; i < buttons.size(); ++i) {
        ...
        // Use the project root we already found at the top of the function
        if (!projectRoot.isEmpty()) {
...
void Dynamic::resizeEvent(QResizeEvent *event)
{
    ...
    // If the fullscreen video is visible, ensure it also resizes correctly to fill entire screen
    if (fullscreenStackWidget && fullscreenStackWidget->isVisible()) {
        QWidget* mainWindow = window();
        if (!mainWindow) {
            mainWindow = this;
        }
        
        QRect targetRect = mainWindow->geometry();
        ...
        // Ensure fullscreen widget is parented to main window
        if (fullscreenStackWidget->parent() != mainWindow) {
            fullscreenStackWidget->setParent(mainWindow);
        }
        
        fullscreenStackWidget->setGeometry(QRect(QPoint(0, 0), targetRect.size()));
        fullscreenVideoWidget->setGeometry(fullscreenStackWidget->rect());
        qDebug() << "Dynamic::resizeEvent - Resized fullscreenVideoWidget to fill entire screen:" << targetRect;
    }
...
bool Dynamic::eventFilter(QObject *obj, QEvent *event)
{
    ...
    if (event->type() == QEvent::MouseButtonPress) {
        ...
        if (mouseEvent->button() == Qt::LeftButton) {
            ...
            // If the click is on the fullscreen stack widget (but not the video itself), allow it to pass through
            // This ensures clicks on the back button are handled normally
            if (obj == fullscreenStackWidget && fullscreenStackWidget->isVisible()) {
                // Check if click is on the back button
                if (ui->back && ui->back->isVisible()) {
                    QPoint clickPos = mouseEvent->pos();
                    QPoint globalClickPos = fullscreenStackWidget->mapToGlobal(clickPos);
                    QPoint backButtonGlobalPos = ui->back->mapToGlobal(QPoint(0, 0));
                    QRect backButtonRect(backButtonGlobalPos, ui->back->size());
                    
                    if (backButtonRect.contains(globalClickPos)) {
                        // Click is on back button, let it handle the event normally
                        qDebug() << "Dynamic::eventFilter - Click on back button while fullscreen visible.";
                        return false; // Let the back button handle it
                    }
                }
                // Click is on fullscreen widget but not on video or back button
                // This means click is on the video area, so navigate to capture
                qDebug() << "Dynamic::eventFilter - Click on fullscreen widget (video area). Emitting videoSelectedAndConfirmed().";
                fullscreenPlayer->stop();
                emit videoSelectedAndConfirmed(m_selectedVideoPath);
                hideOverlayVideo();
                return true;
            }
            
            // If the click is on the main Dynamic widget (this) while fullscreen video is visible
            // This handles clicks outside the explicit video area
            if (obj == this && fullscreenStackWidget && fullscreenStackWidget->isVisible()) {
                QPoint clickPos = mouseEvent->pos();
                ...
                QRect fullscreenRect = fullscreenStackWidget->geometry();

                qDebug() << "Dynamic::eventFilter - Click on main widget while fullscreen visible.";
                qDebug() << "Click pos:" << clickPos << "Fullscreen rect:" << fullscreenRect
                         << "Back button rect:" << backButtonRect;

                // If the click is outside the fullscreen video and back button
                if (!fullscreenRect.contains(clickPos) && !backButtonRect.contains(clickPos)) {
...
void Dynamic::processVideoClick(QObject *buttonObj)
{
    ...
    if (!actualVideoPath.isEmpty()) {
        // Store the selected video path for the signal
        m_selectedVideoPath = actualVideoPath;
        
        // Stop all GIFs before navigating to capture page
        QTimer::singleShot(0, [this, actualVideoPath]() {
            ...
            // Directly navigate to capture page without showing preview
            qDebug() << "Dynamic::processVideoClick - Navigating directly to capture page with video:" << actualVideoPath;
            emit videoSelectedAndConfirmed(actualVideoPath);
        });
    } else {
...
void Dynamic::showOverlayVideo(const QString& videoPath)
{
    ...
    if (!fullscreenPlayer || !fullscreenVideoWidget) {
        qWarning() << "showOverlayVideo: Essential components are null (player or widget).";
        return;
    }

    // Get the main window to fill the entire screen
    QWidget* mainWindow = window();
    if (!mainWindow) {
        mainWindow = this; // Fallback to self if no window
    }
    
    // Get the geometry of the main window to fill the entire screen
    QRect targetRect = mainWindow->geometry();
    if (targetRect.isEmpty()) {
        // If window geometry is empty, try to get screen geometry
        QScreen* screen = QApplication::primaryScreen();
        if (screen) {
            targetRect = screen->availableGeometry();
        } else {
    ...
    // Convert to coordinates relative to the main window
    QPoint windowPos = mainWindow->mapToGlobal(QPoint(0, 0));
    QRect globalRect(windowPos, targetRect.size());
    
    // Position and size the container widget to cover the entire screen
    // Reparent to main window to ensure it covers everything
    fullscreenStackWidget->setParent(mainWindow);
    fullscreenStackWidget->setGeometry(QRect(QPoint(0, 0), targetRect.size()));
    ...
    // Show the stack widget (which contains the video)
    fullscreenStackWidget->show();
    fullscreenStackWidget->raise(); // Bring to front

    // Keep the original back button visible and raise it above the video
    // Reparent it to main window so it's above the fullscreen widget
    if (ui->back) {
        // Get the back button's position relative to the main window
        QPoint backButtonPos = ui->back->mapToGlobal(QPoint(0, 0));
        QPoint windowPos = mainWindow->mapToGlobal(QPoint(0, 0));
        QPoint relativePos = backButtonPos - windowPos;
        
        // Reparent to main window to ensure it's above the fullscreen widget
        ui->back->setParent(mainWindow);
        ui->back->move(relativePos);
        ui->back->show(); // Ensure it's visible
        ui->back->raise(); // Raise it above the video widget
        ui->back->setAttribute(Qt::WA_TransparentForMouseEvents, false); // Ensure it's clickable
        qDebug() << "Dynamic::showOverlayVideo - Back button reparented to main window and raised above video";
        qDebug() << "Dynamic::showOverlayVideo - Back button position:" << relativePos;
    }
    
    // Hide the separate video preview back button since we're using the original
    if (videoPreviewBackButton) {
        videoPreviewBackButton->hide();
    }
    
    qDebug() << "Dynamic::showOverlayVideo - Fullscreen video displayed covering entire screen";
    qDebug() << "Dynamic::showOverlayVideo - Fullscreen stack widget geometry:" << fullscreenStackWidget->geometry();
    qDebug() << "Dynamic::showOverlayVideo - Back button visible:" << (ui->back ? ui->back->isVisible() : false);
}
...
void Dynamic::hideOverlayVideo()
{
    ...
    // Hide video preview back button (if it was shown)
    if (videoPreviewBackButton) {
        videoPreviewBackButton->hide();
    }
    
    // Restore original back button to its original parent (Dynamic widget)
    if (ui->back) {
        QWidget* mainWindow = window();
        if (mainWindow && ui->back->parent() == mainWindow) {
            // Get the back button's current position relative to main window
            QPoint currentPos = ui->back->pos();
            
            // Reparent back to Dynamic widget
            ui->back->setParent(this);
            
            // Convert position back to Dynamic widget coordinates
            QPoint globalPos = mainWindow->mapToGlobal(currentPos);
            QPoint localPos = this->mapFromGlobal(globalPos);
            ui->back->move(localPos);
        }
        
        ui->back->show();
        ui->back->raise();
        qDebug() << "Dynamic::hideOverlayVideo - Back button restored to Dynamic widget";
    }
