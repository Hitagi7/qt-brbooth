...
void SegmentationManager::processOptimizedSegmentation(const cv::Mat& frame, const QList<OptimizedDetection>& detections) {
    ...
    try {
        ...
        // Convert OptimizedDetection to BoundingBox for processing
        QList<BoundingBox> boundingBoxes;
        for (const OptimizedDetection& detection : detections) {
            BoundingBox bbox;
            bbox.x1 = detection.boundingBox.x;
            bbox.y1 = detection.boundingBox.y;
            bbox.x2 = detection.boundingBox.x + detection.boundingBox.width;
            bbox.y2 = detection.boundingBox.y + detection.boundingBox.height;
            bbox.confidence = detection.confidence;
            boundingBoxes.append(bbox);
        }
        
        // Process segmentation using the regular processor
        QList<SegmentationResult> results = m_segmentationProcessor->segmentPersons(frame, boundingBoxes, m_confidenceThreshold);
