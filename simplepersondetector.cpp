...
QList<SimpleDetection> SimplePersonDetector::detect(const cv::Mat& image) {
    ...
    try {
        ...
        m_hog.detectMultiScale(
            ...
            weights,
            -0.3,
            cv::Size(16, 16), // Larger stride for faster processing
            cv::Size(8, 8),   // Larger padding for faster processing
            1.1,              // Larger scale factor for faster processing
            0.5,              // Higher threshold for fewer detections
            false
        );
